...
def lowest_common_ancestor(goterms, godag):
	...
	parents  = all_common_parent_go_ids(goterms, godag)
	if len(parents) < 1:
		return None
	return max(parents, key=lambda t: godag[t].depth)
...
def Similarity_of_Two_GOTerms(go_id1, go_id2, go, method):
	if method == 'Baseline_LCA':
		...
		print(f"Sim down {sim_down}")
		print(f"Sim up {sim_upper}")
		#sim = (sim_down*0.3) + (sim_upper*0.7)
		#if sim > 1:
	#		sim = 1
			#raise RuntimeError("Above 1")
		return sim
...
def Similarity_of_Set_of_GOTerms(set1, set2, method):
    sem1 = [max([(g1, g2, Similarity_of_Two_GOTerms(g1, g2, go, method)) for g2 in set2], key=lambda x: x[2]) for g1 in set1]
    sem2 = [max([(g2, g1, Similarity_of_Two_GOTerms(g2, g1, go, method)) for g1 in set1], key=lambda x: x[2]) for g2 in set2]
    return round((sum(x[2] for x in sem1 + sem2) / (len(set1) + len(set2))), 3)


def Similarity_of_Set_of_GOTerms_old(set1, set2, method):
	Sim1 = []
	Sim2 = []
